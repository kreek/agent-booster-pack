...
- Release coordination or changelog process; use `deployment-and-cicd`.
- Alert mechanics and dashboards; use `observability-for-services`.
...
- Use `api-design` for OpenAPI and wire-contract shape.
- Use `observability-for-services` for alert/runbook signal definitions.
- Use `git-workflow-depth` or `smart-commit` when documenting commit/PR history
  or release notes.
