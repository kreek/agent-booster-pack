...
- Internal function signatures with no caller contract; use `data-first-design`
  or `refactoring-safely`.
- Auth, secrets, or trust-boundary review beyond API shape; use
  `security-review`.
- Database schema design; use `database-safety`.
...
- Use `proof-driven-engineering` when API claims need explicit proof
  obligations.
- Use `security-review` for authn/authz, input trust, SSRF, secrets, and data
  exposure.
- Use `distributed-systems-resilience` for remote-call retries, idempotent
  consumers, and delivery guarantees.
