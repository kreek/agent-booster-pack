...
- Error type design; use `error-handling-patterns`.
- Deployment sequencing; use `deployment-and-cicd`.
...
- Use `deployment-and-cicd` for rollout gates and production verification.
- Use `distributed-systems-resilience` for remote dependency failure behavior.
