...
- Remote-call resilience; use `distributed-systems-resilience`.
- Performance measurement without concurrency design changes; use
  `performance-profiling`.
- Database transaction isolation; use `database-safety`.
...
- Use `data-first-design` to remove shared mutable state from the core design.
- Use `observability-for-services` when concurrency failure needs metrics,
  traces, or saturation dashboards.
- Use `debugging-methodology` for existing races or deadlocks before changing
  code.
