...
- Performance investigation where the symptom is slowness; use `performance`.
...
- Use `git` for `git bisect`, reflog recovery, or conflict-heavy debugging.
- Use `observability` when the right evidence must come from logs, metrics,
  traces, or incident timelines.
