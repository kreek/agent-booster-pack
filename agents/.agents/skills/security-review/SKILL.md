...
- General code quality with no trust boundary; use the relevant engineering
  skill.
- API shape without security semantics; use `api-design`.
- Runtime alert design; pair with `observability-for-services`.
...
- Use `error-handling-patterns` for safe error propagation and user-facing
  failure shape.
- Use `api-design` for auth/error/idempotency contract shape.
