...
- Backend API shape; use `api-design`.
- Frontend runtime debugging or tests only; pair with `behavior-testing` and
  browser tooling.
- Performance profiling beyond UI design choices; use `performance-profiling`.
...
- Use `behavior-testing` for UI behavior tests and browser-verified flows.
- Use `performance-profiling` for measured Core Web Vitals or rendering
  regressions.
