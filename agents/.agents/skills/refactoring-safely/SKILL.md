...
- Behavior-first feature work; use `behavior-testing`.
- Commit grouping after changes already exist; use `smart-commit`.
- Git history surgery; use `git-workflow-depth`.
...
- Use `proof-driven-engineering` when refactor safety depends on explicit
  preservation evidence.
- Use `behavior-testing` for characterization and boundary tests.
- Use `smart-commit` to group resulting changes cleanly.
- Use `data-first-design` when the refactor is mainly about untangling effects
  and domain shape.
