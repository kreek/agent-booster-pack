...
- Remote-call retry policy; use `distributed-systems-resilience`.
- Security-specific failure shape; pair with `security-review`.
- Observability of errors in production; pair with `observability-for-services`.
...
- Use `security-review` for auth, secrets, validation, fail-closed behavior, and
  information disclosure.
- Use `distributed-systems-resilience` for retry budgets, idempotency keys, and
  circuit breakers.
- Use `observability-for-services` for correlation IDs, logs, traces, and
  error-rate alerts.
