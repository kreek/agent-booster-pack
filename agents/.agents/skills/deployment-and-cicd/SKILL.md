...
- Database DDL/data safety itself; pair with `database-safety`.
- Service monitoring and alert design; pair with `observability-for-services`.
...
- Use `database-safety` for migration mechanics and lock/backfill risk.
- Use `observability-for-services` for rollout metrics, dashboards, alerts, and
  runbooks.
- Use `security-review` for CI credentials, artifact signing, SBOMs, and
  dependency trust.
