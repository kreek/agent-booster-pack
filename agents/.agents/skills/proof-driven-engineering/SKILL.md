...
- Pure formatting, typo fixes, or mechanical file moves with no behavior, data,
  or contract claim.
- Commit grouping after evidence already exists; use `smart-commit`.
- Test-shape decisions only; use `behavior-testing`.
...
- Use `data-first-design` to shape invariants and make invalid states
  unrepresentable.
- Use `behavior-testing` to choose proof boundaries and test names.
- Use `debugging-methodology` when the proof depends on root-cause evidence.
- Use `api-design` when the claim is a public contract.
- Use `refactoring-safely` when the proof is behavior preservation through
  structural change.
