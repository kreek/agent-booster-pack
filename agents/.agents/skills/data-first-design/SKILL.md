...
- Public HTTP contract details; use `api-design`.
- Database physical schema, indexes, or migrations; use `database-safety`.
- Broad refactoring sequence; use `refactoring-safely`.
...
- Use `proof-driven-engineering` when data claims need explicit proof
  obligations.
- Use `behavior-testing` to prove domain behavior through public boundaries.
- Use `error-handling-patterns` for parse failures, Result/Either shape, and
  error context.
- Use `concurrency-patterns` when mutable places or ownership cross task/thread
  boundaries.
